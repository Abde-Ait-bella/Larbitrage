...
function Matches() {
    ...
    const handleDelete = (id) => {
        ...
        axios.delete(`http://localhost:8000/api/matche/${id}`)
            .then((response) => {
                // Handle success, e.g., update your state or perform any necessary actions
                console.log(`Matche with id ${id} deleted successfully`);
            })
            .catch((error) => {
                // Handle error, e.g., show an error message
                console.error(`Error deleting Matche with id ${id}:`, error);
            });
    };
            ...
            <div class="container-fluid pt-4 px-4">
                <div class="bg-secondary text-center rounded p-4">
                    <div class="d-flex align-items-center justify-content-between mb-3">
                        ...
                        {/* <a href="">Show All</a> */}
                        <Link to="/addRapport" class="btn btn-warning pe-3">إضافة مباراة <i class="fa-solid fa-circle-plus me-2"></i></Link>
                    </div>
                            ...
                            <tbody>
                                {matches?.map((m) => (
                                    <tr className="text-center" key={m.id}>
                                        ...
                                        <td><Link to='/matches' onClick={() => handleDelete(m.id)} ><i class="fa-solid fa-trash"></i></Link></td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
